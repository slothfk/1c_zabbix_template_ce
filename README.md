...
* Шаблон для мониторинга сервера лицензирования (реализовано);
* Шаблон для мониторинга центральных серверов кластера 1С Предприятия (планируется к реализации).
...
* {$LIC_CORP} - флаг использования лицензий КОРП (1 - используются КОРП, 0 - используются ПРОФ). Для чего это нужно, можно ознакомиться по ссылке https://infostart.ru/redirect.php?url=aHR0cHM6Ly9pdHMuMWMucnUvZGIvdjgzMTRkb2MjYm9va21hcms6YWRtOlRJMDAwMDAwOTcx;
